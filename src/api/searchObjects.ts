...
export async function searchObjects(coordinates: { lat: number; lng: number }, radius: number, filters: Array<IFilterIcon>) {
    try {
        ...
        while (page<=5) {
            const response = await axios.get(`https://catalog.api.2gis.com/3.0/items?${filters_str}&type=branch&point=${coordinates.lng},${coordinates.lat}&page=${page}&fields=items.point,items.rubrics,items.description,items.external_content,items.schedule,items.full_address_name&radius=${radius}&key=${API_KEY}`);
